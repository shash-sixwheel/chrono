...
// -----------------------------------------------------------------------------
Revoy::Revoy(CollisionType chassis_collision_type,
             ChContactMethod contactMethod)
    : ChWheeledVehicle("Revoy", contactMethod) {
  Create(chassis_collision_type);
}
...
void Revoy::Create(CollisionType chassis_collision_type) {
  ...
  m_axles[0]->m_suspension =
      chrono_types::make_shared<Kraz_tractor_FrontSuspension>("Suspension");

  m_axles[0]->m_wheels.resize(4);
  m_axles[0]->m_wheels[0] =
      chrono_types::make_shared<Kraz_tractor_Wheel>("Wheel_Li");
  m_axles[0]->m_wheels[1] =
      chrono_types::make_shared<Kraz_tractor_Wheel>("Wheel_Lo");
  m_axles[0]->m_wheels[2] =
      chrono_types::make_shared<Kraz_tractor_Wheel>("Wheel_Ri");
  m_axles[0]->m_wheels[3] =
      chrono_types::make_shared<Kraz_tractor_Wheel>("Wheel_Ro");

  m_axles[0]->m_brake_left =
      chrono_types::make_shared<Kraz_tractor_Brake>("Brake_L");
  m_axles[0]->m_brake_right =
      chrono_types::make_shared<Kraz_tractor_Brake>("Brake_R");
...
void Revoy::Initialize(std::shared_ptr<ChChassis> frontChassis,
                       const ChCoordsys<> &chassisPos, double chassisFwdVel) {
  ...
  m_steerings[0]->Initialize(m_chassis, ChVector3d(0, 0, 0),
                             ChQuaternion<>(1, 0, 0, 0));

  // Initialize the axle subsystem.
  const double twin_tire_dist = 0.33528; // Michelin for 12.00 R 20
  m_axles[0]->Initialize(m_chassis, nullptr, m_steerings[0],
                         ChVector3d(-6, 0, 0), ChVector3d(0), twin_tire_dist);
  ...
  std::shared_ptr<ChEngine> engine =
      chrono_types::make_shared<Revoy_EngineSimpleMap>("Revoy_Engine");
  std::shared_ptr<ChTransmission> transmission =
      chrono_types::make_shared<Revoy_AutomaticTransmissionSimpleMap>(
          "Revoy_Transmission");
